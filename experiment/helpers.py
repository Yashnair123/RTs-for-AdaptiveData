...
#     for crop in range(3):
#         data=[]
#         years_by_action = [[] for _ in range(4)]
#         for t in range(len(full_data)):
#             if full_data[t][0] == crop:
#                 data.append([full_data[t][1], full_data[t][2]])
#                 years_by_action[int(full_data[t][1])].append(list_of_years[t])
#         T = len(data)
...
#             # times_by_action[data[t][0]].append(years_by_action[int(full_data[t][1])][t])
...
#                 X = np.array(years_by_action[action])[:,np.newaxis]
...
#         #years_by_action = [[] for _ in range(4)]
#         times_by_action = [[] for _ in range(4)]
#         times_by_action_counter = np.zeros(4)
#         for t in range(len(full_data)):
#             if full_data[t][0] == crop:
#                 data.append([full_data[t][1], full_data[t][2]])
#                 times_by_action[int(full_data[t][1])].append(times_by_action_counter[int(full_data[t][1])])
#                 times_by_action_counter[int(full_data[t][1])] += 1
...
#         abs_betas = []
#         for action in range(4):
#             if len(responses_by_action[action]) > 0:
#                 X = np.array(times_by_action[action])[:,np.newaxis]
#                 y = responses_by_action[action]
#                 reg = LinearRegression().fit(X, y)
#                 abs_beta = reg.coef_[0]
#             else:
#                 abs_beta = 0.
            
#             abs_betas.append(abs_beta)

#         test_stats.append(np.max(abs_betas))
